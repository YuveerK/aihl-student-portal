...
const EditDocumentModal = ({
  handleCloseUpdateDocumentModal,
  learnerId,
  document,
}) => {
  ...
  const [file, setFile] = useState(document.document.documentName);
  ...
  const handleAddDocument = async (e) => {
    ...
    if (!documentName || !category || !file) {
      ...
      formData.append("studentDocumentId", document.document.studentDocumentId);
      formData.append("file", file);

      try {
        // Send the form data to the backend to handle the file upload
        await axios.put(
          `http://localhost:2000/files/${document.document.studentDocumentId}`,
          formData,
          {
            headers: {
    ...
    <div className="w-full h-screen absolute top-0 left-0 bg-black/50 flex items-center justify-center">
      <form className="w-[800px] h-auto overflow-auto bg-blue-950 p-8 rounded-md">
        <div className="w-full flex items-center justify-end">
          <div onClick={() => handleCloseUpdateDocumentModal(false)}>
            <AiOutlineClose
              size={25}
              color="white"
